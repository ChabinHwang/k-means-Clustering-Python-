...
def new_center_K(dictionary,data_arr,K):                            #그룹 중심들 다시 잡기
     ...
     for a in range(K):                                             #0~8까지 중심에 대해 중심 재정의
          ...
          for i in range(len(dictionary[a])):                       #i는 각 딕셔너리 벨류값
               arr_temp=np.add(arr_temp,data_arr[dictionary[a][i]])	#arr_temp에 그룹원들의 벡터값의 합을 저장,j는 키값(중심)i는 원소
          arr=np.divide(arr_temp,len(dictionary[j]))                #temp를 백터수로나눠서 새로운 중심을 지정(arr3)중요
...
for m in range(len(data_arr)-1):                          #500개의 문서를 9개의 중심들과 거리 비교 후, 가장 짧은 곳 키인덱스:문서인덱스 딕셔너리에 넣음
        dist=[vector_distance_calculate(center_arr[i]-data_arr[m+1])for i in range(K)]#i부터 k까지, 거리비교
        dictionary[np.argmin(dist)].append(m+1)       # 가장 작은 값의 인덱스(0~8중하나), m은 문서번호(배열상의)를 의미함
